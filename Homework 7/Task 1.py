...
class Segment:
     ...
     def length(self):
         sgm1 = ((self.point2[0] - self.point1[0])**2 + (self.point2[1] - self.point1[1])**2)**0.5
         sgm2 = round(sgm1, 2)
         return sgm2

     def x_axis_intersection(self):
         if (self.point2[1] <= 0 <= self.point1[1]) or (self.point1[1] <= 0 <= self.point2[1]):
            return True
         else: return False
     def y_axis_intersection(self):
         if (self.point2[0] <= 0 <= self.point1[0]) or (self.point1[0] <= 0 <= self.point2[0]):
            return True
         else: return False
