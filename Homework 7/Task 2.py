...
class PersonInfo:
    ...
    def short_name(self):
        cut_fio = self.person[0].split(' ')
        short_fio = f"{cut_fio[1]} {cut_fio[0][0]}."
        return short_fio

    def path_deps(self):
        department_tpl = list(self.person[2:])
        department_path = ' --> '.join(department_tpl)
        return department_path

    def new_salary(self):
        string = ''
        for word in self.person[2:]:
            string+=word
        letters_dict = {ltr: string.count(ltr) for ltr in set(string)}
        lst_letter = sorted(letters_dict.items(), key = lambda x: x[1], reverse=True)
        sum_letters = lst_letter[0][1] + lst_letter[1][1] + lst_letter[2][1]
        salary = 1337 * self.person[1] * sum_letters
        return salary
